...
SELECT FIRST_NAME, SUM(SALARY) FROM EMPLOYEES; --일반 컬럼과 그룹함수는 출력 불가능
...
--GROUPING() - 그룹절로 생성되면 0 반환, 롤업 OR 큐브로 만들어지면 1 반환
...
--문제 6.
--부서별, JOB_ID를 그룹핑 하여토탈, 합계를 출력하세요.
--GROUPING() 을 이용하여
--소계 합계를 표현하세요
SELECT DECODE(GROUPING(DEPARTMENT_ID), 1, '합계', DEPARTMENT_ID) AS DEPRATMENT_ID,
