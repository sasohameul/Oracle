...
--BETWEEN~AND(이상~이하), IN(정확히 일치하는), LIKE(포함된)
...
SELECT * FROM EMPLOYEES WHERE DEPARTMENT_ID IN(10,20,30,40,50);--값이 정확히 일치, 많이 사용됨
...
SELECT * FROM EMPLOYEES WHERE HIRE_DATE LIKE '___05%'; --언더바 세개, 05앞에 특수문자 포함 세글자가 나온다.
...
SELECT * FROM EMPLOYEES WHERE (JOB_ID = 'IT_PROG' OR JOB_ID = 'FI_MGR') AND SALARY >= 6000; --FI, IT중 6000이상인 사람  

--------------------------------------------------------------------------------

--ORDER BY 컬럼(엘리어스)
SELECT * FROM EMPLOYEES ORDER BY HIRE_DATE; --날짜 기준 ASC(기본)
...
--정렬 여러개 ,로 나열
SELECT FIRST_NAME, SALARY, JOB_ID FROM EMPLOYEES ORDER BY JOB_ID ASC, SALARY DESC;-JOBID 먼저 오름차순정렬 후, 그 중 같은 아이디 중 SALARY 내림차순정렬
