...
SELECT FIRST_NAME, MANAGER_ID WHERE DEPARTMENTS_ID = 50;
...
SELECT FIRST_NAME, SALARY FROM EMPLOYEES WHERE DEPARTMENT_ID = 60 ORDER BY SALARY ASC;
...
SELECT FIRST_NAME || '사원의 급여는 ' || SALARY ||'달러 입니다' FROM EMPLOYEES WHERE EMPLOYEE_ID = '100';

--14. 매니저(MAN) 직무에 해당하는 사원의 이름과 직무아이디를 출력하세요. (컬럼:job_id)
SELECT FIRST_NAME, JOB_ID FROM EMPLOYEES WHERE JOB_ID LIKE '___MAN%';

--15. 매니저(MAN) 직무에 해당하는 사원의 이름과 직무아이디를 직무아이디 순서대로 출력하세요.
SELECT*FROM EMPLOYEES;
SELECT FIRST_NAME, JOB_ID FROM EMPLOYEES WHERE JOB_ID LIKE '___MAN%' ORDER BY JOB_ID;